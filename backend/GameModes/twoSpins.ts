...
export class TwoSpinsGameMode implements GameMode {
    ...
    async generate(options: GeneratorOptions, players: string[]): Promise<GameModeDetails> {
        ...
        const spin2 = await this.generateSpin(spinGenOptions, options['noTargets'] as boolean);
    ...
    async handleAdminEvent(event: string, payload: AdminEventPayload, currentState: GameModeDetails): Promise<GameModeDetails> {
        ...
        if (event === "respin") {
            (currentState['currentSpins'] as Spin[])[payload.map] = await this.generateSpin(currentState['generatorOptions'] as SpinGeneratorOptions, currentState['noTargets'] as boolean);
        }
        return currentState;
    }
